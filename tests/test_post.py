...
def test_unauthorized_user_get_all_posts(client, test_posts):
    ...
    assert res.status_code == 404 #changed this one from 401
...
def test_get_one_post_not_exist(authorized_client, test_posts):
    res = authorized_client.get("/posts/55555")
    assert res.status_code == 404
    assert res.json().get("detail") == "Post with id: 55555 was not found"
...
@pytest.mark.parametrize("title, content, published", [
    ("titleFirst", "contentFirst", True),
    ("titleSecond", "contentSecond", False),
    ("titleThird", "contentThird", True)    
])   
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts/", json={"title":"testing title",
                                                  "content":"testing content"})
    
    created_post = schemas.Post(**res.json())
    
    assert res.status_code == 201
    assert created_post.title == "testing title"
...
def test_unauthorized_user_create_post(client, test_user, test_posts):
    res = client.post("/posts/", json={"title":"testing title",
                                                  "content":"testing content"})
    assert res.status_code == 401
...
def test_delete_post_non_exist(authorized_client, test_posts, test_user):
    res = authorized_client.delete("/posts/55555")
    assert res.status_code == 404
...
def test_update_post_non_exist(authorized_client, test_posts, test_user):
    ...
    res = authorized_client.put(f"/posts/55555", json=data)
    assert res.status_code == 404